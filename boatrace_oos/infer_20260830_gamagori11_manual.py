...
cards=read_csv(SRC/'data/programs/race_cards/2026/08/30.csv'); cd={str(r['レースコード']):r for _,r in cards.iterrows()}
code='202608300711'; venue=7; rno=11; card=cd[code]
tkz=pd.Series({'艇1_展示タイム':'6.77','艇2_展示タイム':'6.72','艇3_展示タイム':'6.76','艇4_展示タイム':'6.79','艇5_展示タイム':'6.80','艇6_展示タイム':'6.83'})
# Official water/weather if available; screenshot fallback: wind 2m, wave 1cm, cloudy, opposite direction vs prior race.
sui_all=read_csv(SRC/'data/previews/sui/2026/08/30.csv')
sui=None
if not sui_all.empty and 'レースコード' in sui_all.columns:
 rows=sui_all[sui_all['レースコード'].astype(str)==code]
 if not rows.empty: sui=rows.iloc[0]
if sui is None:
 sui=pd.Series({'風速(m)':'2','波の高さ(cm)':'1','風向':'1','天候':'2'})
print('SUI_RAW',dict(sui),flush=True)
