...
code='202608280909'; venue=9; rno=9; card=cd[code]
print('PRESENT',code in cd,code in td,code in sd,code in od,flush=True)
manual_tkz={f'艇{i}_展示タイム':v for i,v in enumerate([6.75,6.80,6.81,6.87,6.87,6.82],1)}
...
# Report actual right-arrow condition as code 3 if manual fallback.
actual_sui=raw_sui_list[0] if len(raw_sui_list)==1 else {'風速(m)':1.0,'波の高さ(cm)':1.0,'風向':3,'天候':2}
x,err=build_feature_frame(card,raw_tkz,actual_sui,state,venue,rno); assert err is None,err
