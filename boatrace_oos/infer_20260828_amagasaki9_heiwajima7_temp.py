...
cases=[
 ('AMAGASAKI9','202608281309',13,9,[6.90,6.91,6.80,6.90,6.86,6.89],0.0,0.0,2,None),
 ('HEIWAJIMA7','202608280407',4,7,[6.79,6.84,6.85,6.70,6.90,6.84],3.0,3.0,1,3),
]
perm=['-'.join(map(str,p)) for p in itertools.permutations(range(1,7),3)]
for label,code,venue,rno,exh,ws,wave,weather,actual_w in cases:
 print('\nCASE',label,code,flush=True); card=cd[code]
 ...
 if code in td and code in sd:
  ...
  raw_tkz=manual_tkz
  if actual_w is None: raw_sui_list=[{'風速(m)':ws,'波の高さ(cm)':wave,'風向':np.nan,'天候':weather}]
  else: raw_sui_list=[{'風速(m)':ws,'波の高さ(cm)':wave,'風向':w,'天候':weather} for w in range(1,9)]
  print('USING_SCREENSHOT_PREVIEW',flush=True)
 results=[]
 for raw_sui in raw_sui_list:
  x,err=build_feature_frame(card,raw_tkz,raw_sui,state,venue,rno); assert err is None,err
  ...
  results.append((raw_sui,x,strength,topprob,combos,p4,cls))
  print('WIND_INPUT',raw_sui.get('風向'),'P4',repr(p4),'CLASS',cls,'TOP4','|'.join(combos),'TOPPROB','|'.join(f'{v:.10f}' for v in topprob),flush=True)
 if len(results)>1: chosen=next(z for z in results if z[0].get('風向')==actual_w)
 else: chosen=results[0]
 raw_sui,x,strength,topprob,combos,p4,cls=chosen
