...
cases=[
 ('BIWAKO9','202608281109',11,9,[6.75,6.68,6.77,6.79,6.71,6.75],1.0,1.0,1,5),
 ('TAMAGAWA7','202608280507',5,7,[6.80,6.76,6.82,6.82,6.88,6.70],4.0,3.0,2,3),
]
...
for label,code,venue,rno,exh,ws,wave,weather,actual_w in cases:
 ...
 if code in td and code in sd:
  ...
  raw_tkz=manual_tkz; raw_sui_list=[{'風速(m)':ws,'波の高さ(cm)':wave,'風向':w,'天候':weather} for w in range(1,9)]; print('USING_SCREENSHOT_PREVIEW',flush=True)
 results=[]
 ...
 if len(results)>1:
  chosen=next(z for z in results if z[0].get('風向')==actual_w)
