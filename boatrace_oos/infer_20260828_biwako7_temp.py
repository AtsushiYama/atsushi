...
code='202608281107'; venue=11; rno=7
# User screenshot 13:10: exhibit 6.74/6.82/6.75/6.71/6.66/6.80, cloudy, wind 1m arrow right, wave 1cm.
tkz={f'艇{i}_展示タイム':v for i,v in enumerate([6.74,6.82,6.75,6.71,6.66,6.80],1)}
sui={'風速(m)':1.0,'波の高さ(cm)':1.0,'風向':2,'天候':2}
x,err=build_feature_frame(cd[code],tkz,sui,state,venue,rno); assert err is None,err
...
print('P4',repr(p4),flush=True); print('CLASS',cls,flush=True); print('TOP4','|'.join(combos),flush=True); print('TOPPROB','|'.join(f'{v:.10f}' for v in topprob),flush=True); print('STRENGTHS','|'.join(f'{v:.8f}' for v in strength),flush=True); print('EXHIBIT','|'.join(f'{v:.2f}' for v in x.sort_values('boat')['exhibit'].to_numpy()),flush=True); print('WIND_SPEED',x['wind_speed'].iloc[0],'WAVE',x['wave'].iloc[0],'WEATHER_CODE',x['weather'].iloc[0],'WIND_DIR_CODE',x['wind_dir'].iloc[0],flush=True)
if code in od:
