...
code='202608281311'; card=cd[code]; exh=[6.84,6.86,6.82,6.92,6.81,6.89]
manual_tkz={f'艇{i}_展示タイム':v for i,v in enumerate(exh,1)}
print('CASE AMAGASAKI11',code,flush=True); print('PRESENT',code in cd,code in td,code in sd,code in od,flush=True)
if code in td and code in sd:
 raw_tkz=td[code]; raw_sui_list=[sd[code]]; print('USING_OFFICIAL_PREVIEW',flush=True)
else:
 raw_tkz=manual_tkz; raw_sui_list=[{'風速(m)':1.0,'波の高さ(cm)':0.0,'風向':w,'天候':2} for w in range(1,9)]; print('USING_SCREENSHOT_PREVIEW',flush=True)
results=[]
for raw_sui in raw_sui_list:
 x,err=build_feature_frame(card,raw_tkz,raw_sui,state,13,11); assert err is None,err
...
chosen=results[0] if len(results)==1 else results[0]
