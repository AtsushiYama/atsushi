...
code='202608280103'; card=cd[code]; exh=[6.83,6.79,6.84,6.94,6.90,6.84]
manual_tkz={f'艇{i}_展示タイム':v for i,v in enumerate(exh,1)}
print('CASE KIRYU3',code,flush=True); print('PRESENT',code in cd,code in td,code in sd,code in od,flush=True)
if code in td and code in sd:
 raw_tkz=td[code]; raw_sui_list=[sd[code]]; print('USING_OFFICIAL_PREVIEW',flush=True)
else:
 raw_tkz=manual_tkz; raw_sui_list=[{'風速(m)':3.0,'波の高さ(cm)':2.0,'風向':w,'天候':2} for w in range(1,9)]; print('USING_SCREENSHOT_PREVIEW',flush=True)
results=[]
for raw_sui in raw_sui_list:
 x,err=build_feature_frame(card,raw_tkz,raw_sui,state,1,3); assert err is None,err
