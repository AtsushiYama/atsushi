...
print('ROLLED_2026_TO_0829',flush=True)
cards=read_csv(SRC/'data/programs/race_cards/2026/08/30.csv'); tkz=read_csv(SRC/'data/previews/tkz/2026/08/30.csv'); sui=read_csv(SRC/'data/previews/sui/2026/08/30.csv')
...
code='202608300106'; venue=1; rno=6
print('PRESENT',code in cd,code in td,code in sd,flush=True)
if code not in cd or code not in td or code not in sd: raise SystemExit('OFFICIAL_PREVIEW_NOT_READY')
x,err=build_feature_frame(cd[code],td[code],sd[code],state,venue,rno); assert err is None,err
...
print('THRESHOLDS','A',repr(A_THR),'S',repr(S_THR),flush=True)
