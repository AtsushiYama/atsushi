...
import gzip,pickle,itertools,hashlib,copy
...
def prescreen_one(card,state,booster,patterns):
    ...
    for _,ex,wsp,wave,weather in patterns:
        ...
        for wind in range(9):
            ...
            if err: continue
            raw=booster.predict(x[FEATURES]); _,_,_,p4=pl_top4(raw,x.boat.to_numpy()); best=max(best,p4); n+=1
    return best,n,best>=A_THR
def odds_map(row):
    vals=[]
    for c in row.index:
...
def main():
    ...
    # Bring state to end of 2026-07-31.
    ...
    for dt in pd.date_range('2026-08-01','2026-08-30',freq='D'):
        ...
        if len(q):
            elig=q[q.eligible]; n=len(elig); hits=int(elig.hit.sum()); stake=int(elig.stake.sum()); ret=float(elig.ret.sum()); roi=ret/stake*100 if stake else np.nan
            sa=len(q); screened=int(q.screen_keep.sum()); fn=sa-screened
        ...
        # only after evaluating the day, roll its results into history
        roll_one_day(state,cards,res)
    df=pd.DataFrame(rows); dd=pd.DataFrame(daily)
    df.to_csv('audit_daily_20260801_0830_detail.csv',index=False); dd.to_csv('audit_daily_20260801_0830_summary.csv',index=False)
    active=dd[dd.stake>0].copy(); loss=active[active.roi<100]
    print('ACTIVE_DAYS',len(active),'LOSS_DAYS',len(loss),flush=True)
    for r in loss.itertuples(): print('LOSS_DAY',r.date,'BUY',r.purchases,'HITS',r.hits,'STAKE',r.stake,'RETURN',f'{r.ret:.0f}','ROI',f'{r.roi:.2f}',flush=True)
    print('TOTAL','BUY',int(active.purchases.sum()),'HITS',int(active.hits.sum()),'STAKE',int(active.stake.sum()),'RETURN',f'{active.ret.sum():.0f}','ROI',f'{active.ret.sum()/active.stake.sum()*100:.2f}' if active.stake.sum() else 'NA',flush=True)
