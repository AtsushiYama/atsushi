...
code='202608281307'; venue=13; rno=7; card=cd[code]
print('PRESENT',code in cd,code in td,code in sd,code in od,flush=True)
# Screenshot values: exhibit 6.82/6.89/6.92/6.89/6.97/6.89, cloudy, wind 3m, wave 0cm.
manual_tkz={f'艇{i}_展示タイム':v for i,v in enumerate([6.82,6.89,6.92,6.89,6.97,6.89],1)}
raw_sui_list=[]
if code in td and code in sd:
 raw_tkz=td[code]; raw_sui_list=[sd[code]]; print('USING_OFFICIAL_PREVIEW',flush=True)
else:
 raw_tkz=manual_tkz; raw_sui_list=[{'風速(m)':3.0,'波の高さ(cm)':0.0,'風向':w,'天候':2} for w in range(1,9)]; print('USING_SCREENSHOT_PREVIEW',flush=True)
last=None
for raw_sui in raw_sui_list:
 x,err=build_feature_frame(card,raw_tkz,raw_sui,state,venue,rno); assert err is None,err
 raw=booster.predict(x[FEATURES]); strength,topprob,combos,p4=pl_top4(raw,x.boat.to_numpy()); cls='S' if p4>=S_THR else 'A' if p4>=A_THR else 'skip'; last=(x,strength,topprob,combos,p4,cls)
 print('WIND_INPUT',raw_sui.get('風向'),'P4',repr(p4),'CLASS',cls,'TOP4','|'.join(combos),'TOPPROB','|'.join(f'{v:.10f}' for v in topprob),flush=True)
if len(raw_sui_list)==1:
 x,strength,topprob,combos,p4,cls=last
 print('STRENGTHS','|'.join(f'{v:.8f}' for v in strength),flush=True)
