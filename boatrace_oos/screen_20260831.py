...
def main():
    ...
    state=roll_state(copy.deepcopy(base),'2026-08-30')
    ...
    print('RESULT_0831 TOTAL',len(ans),'KEEP',len(kept),'EXCLUDE',len(ans)-len(kept),'RATE',f'{len(kept)/len(ans):.6f}',flush=True)
    for v,r,code,b,k,n in sorted(kept,key=lambda x:(int(x[2][-4:-2]),x[1])):
        print('KEEP',v,r,f'{b:.12f}',n,flush=True)
    pd.DataFrame(ans,columns=['venue','race_no','code','max_p4proxy','keep','scenarios']).to_csv('screen_20260831.csv',index=False)
