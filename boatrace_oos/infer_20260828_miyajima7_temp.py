...
import gzip,pickle,hashlib,itertools
...
code='202608281707'; venue=17; rno=7
card=cd[code]
tkz={f'艇{i}_展示タイム':v for i,v in enumerate([6.73,6.78,6.71,6.68,6.68,6.71],1)}
sui={'風速(m)':2.0,'波の高さ(cm)':2.0,'風向':6,'天候':1}
...
if code in od:
 r=od[code]; omap={}
 for c in r.index:
  if c.startswith('3連単_'):
   try: omap[c.replace('3連単_','')]=float(r[c])
   except: pass
 odds=[omap.get(c,float('nan')) for c in combos]
